 ...
 *  3. Histograms rendered as PNG images using built-in Java2D (no external libs).
 ...
 *   - Buffer 2 (Type II): Poisson arrivals, mean IAT = 12ms, service Uniform[2,6]ms
...
public class Simulation {
    ...
    static double expRand(double mean)              { return -mean * Math.log(1 - rng.nextDouble()); }
    static double uniformRand(double lo, double hi) { return lo + rng.nextDouble() * (hi - lo); }
    ...
    static class Packet {
        ...
        Packet(int type, double t) { this.type = type; this.arrivalTime = t; }

        double waitInBuffer() { return serviceStartN1 - arrivalTime; }
        double sojourn1()     { return departN1       - arrivalTime; }
        double sojournN2()    { return departN2       - arrivalN2;   }
    }
    ...
    static class Event implements Comparable<Event> {
        ...
        Event(double t, int type, Object data) { this.time = t; this.type = type; this.data = data; }
        ...
        while (!evq.isEmpty()) {
            ...
            switch (evt.type) {
                ...
                case EVT_ARRIVE_B1: {
                    ...
                    if (!n1Busy) {
                        ...
                        if (started) {
                            ...
                            lenB1 = queueB1.size(); lenB2 = queueB2.size();
                        }
                    }
                    break;
                }
                ...
                case EVT_N1_DONE: {
                    ...
                    if (p.type == 1) st.sojourn1_T1.add(p.sojourn1());
                    else             st.sojourn1_T2.add(p.sojourn1());

                    double w = p.waitInBuffer();
                    if (p.type == 1) st.waitB1.add(w);
                    else             st.waitB2.add(w);
                    ...
                    if (p.type == 1) {
                        ...
                        if (!forward) st.type2Discarded++;
                        else          st.type2Forwarded++;
                    }
                    ...
                    if (forward) {
                        if (lenN2Q > ROUTER_THRESH) {
                            ...
                            p.arrivalN2 = clock;
                            queueN2.add(p); lenN2Q++;
                            for (int i = 0; i < 2; i++) {
                                if (n2Proc[i] == null && !queueN2.isEmpty()) {
                                    Packet np = queueN2.poll(); lenN2Q--;
                    ...
                    n1Busy = false; n1InSvc = 0; n1CurrentPacket = null; n1CurrentType = 0;
                    boolean started = tryStartN1v2(clock, queueB1, queueB2,
                                                   n1ActiveBuffer, n1SlotExpiry, evq);
                    if (started) {
                        n1Busy = true; n1InSvc = 1;
                ...
                case EVT_N2_DONE: {
                    ...
                    n2Proc[pid] = null; n2InSvc--;

                    if (!queueN2.isEmpty()) {
                        Packet np = queueN2.poll(); lenN2Q--;
                        np.serviceStartN2 = clock;
                        n2Proc[pid] = np; n2InSvc++;
                        evq.add(new Event(clock + expRand(MEAN_SVC_N2),
    ...
    //  Updated state is returned via lastStartResult = [packet, activeBuffer, slotExpiry].
    // =========================================================================
    ...
    static boolean tryStartN1v2(double clock,
        ...
        Packet p = null;
        if (activeBuffer == 1) {
            if      (!qB1.isEmpty()) p = qB1.poll();
            else if (!qB2.isEmpty()) p = qB2.poll();
        } else {
            if      (!qB2.isEmpty()) p = qB2.poll();
            else if (!qB1.isEmpty()) p = qB1.poll();
        }
    ...
    static double variance(double[] x, double m) {
        double s = 0; for (double v : x) s += (v - m) * (v - m); return s / (x.length - 1);
    }
    ...
    static void testDistribution(String label, List<Double> data) {
        if (data.size() < 30) { System.out.println("  Not enough data."); return; }
        ...
        if      (Math.abs(cv - 1.0) < 0.15) { System.out.println("   → CV ≈ 1 → consistent with Exponential distribution");
                                               System.out.printf("   → Fitted Exp(λ=%.4f), mean=%.4f ms%n", 1/m, m); }
        else if (cv > 0.55 && cv < 0.85)    { System.out.println("   → CV ≈ 0.71 → consistent with Erlang-2 (k=2) distribution");
                                               System.out.printf("   → Fitted Erlang-2(μ=%.4f per stage), overall mean=%.4f ms%n", 2/m, m); }
        else if (cv < 0.2)                   { System.out.println("   → CV ≈ 0 → consistent with Deterministic / near-uniform distribution"); }
        else if (cv > 1.1)                   { System.out.println("   → CV > 1 → heavy-tailed; may fit Hyper-Exponential"); }
        else                                 { System.out.printf("   → CV=%.3f; does not clearly match standard distributions%n", cv); }
    }
    ...
    public static void main(String[] args) {
        ...
        for (int r = 0; r < NUM_REPLICATIONS; r++) {
            ...
            if (r == NUM_REPLICATIONS - 1) {
                allWaitB1 = st.waitB1;      allWaitB2 = st.waitB2;
                allSoj1T1 = st.sojourn1_T1; allSoj1T2 = st.sojourn1_T2;
        ...
        printCI("(c) Fraction of Type-I pkts that waited in Buffer1",     fracWaitB1);
        printCI("(d) Fraction of Type-II pkts that waited in Buffer2",    fracWaitB2);
